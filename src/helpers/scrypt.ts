...
const Scrypt = {
    /**
         ...
         */
        hash(password: string, options?: { N?: number; maxmem?: number }): string {
         ...
         */
        compare(plainTextPassword: string, hash: string, options?: { N?: number; maxmem?: number }): boolean {
            ...
            if (!/^[0-9a-fA-F]+$/.test(hashedPassword) || hashedPassword.length % 2 !== 0) {
                return false;
            }
            ...
            const clearPasswordBuffer = scryptSync(plainTextPassword, salt, 64, scryptOptions);
